...
def q1():
    ...
    pca = PCA(0.95)
    pca.fit(fifa)    
    return round(pca.explained_variance_ratio_ [0], 3).item()
...
def q3():
    ...
    conters = pca.components_.dot(x)
    return tuple(np.around(conters, 3))
...
target = fifa.Overall
features = fifa.drop(columns = ['Overall'])
...
def q4():
    ...
    return features.columns.values[fit.support_].tolist()
